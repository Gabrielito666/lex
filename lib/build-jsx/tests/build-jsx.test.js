...
const buildJSX = require('../index.js');
...
describe('buildJSX', () => {
    const optionsA = {write: true, minify: true, outfile: path.join(outputDir, 'standart-minify-write.js')};
    const optionsB = {write: true, minify: false, outfile: path.join(outputDir, 'standart-no-minify-write.js')};
    ...
    for(const mode of modes)
    {
        for(const options of [optionsA, optionsB, optionsC, optionsD])
        {
            test(`should build a page with ${mode} and ${options.write ? "write" : "no write"} and ${options.minify ? "minify" : "no minify"}`, async () => {
                if(mode === "standart")
                {
                    const result = await buildJSX.standart(stringCodePath, options);
                ...
                {
                    const result = await buildJSX.layout(layoutJsx, pageJsx, options);
                    if(options.write)
                    {
                        expect(fs.existsSync(options.outfile)).toBe(true);
                    }
                ...
                {
                    const result = await buildJSX.byStringCode(stringCode, testDir, options);
                    if(options.write)
                    {
                        expect(fs.existsSync(options.outfile)).toBe(true);
                    }
